  ...
  Future<List<Map<String, dynamic>>> getNews({
    ...
    try {
      ...
      return List.generate(
        10,
        (index) => {
          'title': '$country - ${city ?? ''} Haber ${index + 1}',
          'description': '''${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year} 
            tarihli $country ${city != null ? '- $city' : ''} bölgesi haberi''',
          'publishedAt': DateTime.now().subtract(Duration(hours: index)).toString(),
          'urlToImage': 'https://picsum.photos/200/100?random=$index',
          'content': '''Bu bir örnek haber içeriğidir. Gerçek haberler için internet 
